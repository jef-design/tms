...
export default function CustomerUpload() {
    ...
    const handleUpload = async () => {
        ...
        try {
            setLoading(true);
            await axiosInstance.patch("/api/upload/customers", formData, {
                headers: {"Content-Type": "multipart/form-data"},
            });
            toast.success('Customer Tagging Updated')
            setFile(null);
        } catch (err) {
            console.error(err);
            toast.error("Uploading Customer Tagging failed");
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className='flex items-center justify-center min-h-screen bg-gray-100 p-4'>
            ...
            richColors
            />
          {loading && <Loading message="Updating customers list"/>}
            <div className='bg-white shadow-lg rounded-lg py-8 px-8 max-w-md w-full'>
                <h2 className='text-2xl font-bold text-gray-800 mb-6 text-center'>Update Customer Tagging</h2>
            ...
            ${loading ? "bg-gray-400 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700"}`}
                ...
                <p className='mt-4 text-sm text-gray-500 text-center'>
                    ...
                    <span className='font-medium'>
                        SALESMAN, CUSTCODE, CUSTOMER, CONTACT NO., LONGITUDE, LATITUDE, TAGGED COMPLETED
                    </span>
                </p>
            </div>
        </div>
