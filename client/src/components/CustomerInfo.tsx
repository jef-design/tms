...
import CustomerTaggedStatus from "./CustomerTaggedStatus";
import map404 from '../assets/map404.png'
...
const CustomerMap = () => {
    ...
    useEffect(() => {
        const fetchCustomer = async () => {
            try {
                const res = await axiosInstance.get<Customer>(`/api/upload/customers/${id}`);
        ...
        <div className='flex flex-col lg:flex-row gap-6 p-6'>
            ...
            <div className='flex-1 rounded-lg overflow-hidden bg-white p-4 shadow-lg'>
                ...
                {!customer?.longitude || !customer?.latitude ? (
                    <div className="flex flex-col items-center">
                      <p className="font-bold text-xl">No Address Yet</p>
                      <img src={map404} alt="map404" />
                    </div>
                ) : (
                    <Map
                    ...
                    >
                        ...
                        {userLocation && (
                            <Marker color='blue' longitude={userLocation.longitude} latitude={userLocation.latitude} />
                        )}
                    </Map>
                )}
            </div>
            ...
            <div className='flex-1'>
                <div className='bg-white shadow rounded-lg p-6 space-y-4'>
                    <h2 className='text-2xl font-bold flex gap-4 items-center'>
                        <FaStoreAlt className='text-blue-300' /> {customer.customer_name || customer.customer_code}
                    </h2>
                    <p>
                    ...
                    <p>
                        <CustomerTaggedStatus status={customer?.taggingStatus} />
                    </p>
